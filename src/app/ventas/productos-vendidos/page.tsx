...
function triggerDownload(pdf: SalePdf) {
  ...
  a.href = url;
  a.download = pdf.fileName || 'productos-vendidos.pdf';
...
export default function ProductosVendidosPage() {
  ...
  useEffect(() => { setPdf(null); setPdfError(null); setExcelError(null); }, [from, to, orderBy]);
  ...
  async function fetchPdf(): Promise<SalePdf | null> {
    ...
    setPdfError(null);
    try {
      const res = await salesApi.productsReport(from, to, orderBy);
      setPdf(res);
      return res;
    } catch (err) {
  ...
  async function handleSendEmail() {
    ...
    setSendResult(null);
    try {
      await salesApi.sendProductsReportEmail(email.trim(), from, to, orderBy);
  ...
  async function handleExcelDownload() {
    ...
    setExcelError(null);
    try {
      const res = await salesApi.productsReportExcel(from, to, orderBy);
  ...
  async function handleExcelEmail() {
    ...
    try {
      const res = await salesApi.sendProductsReportExcelEmail(email.trim(), from, to, orderBy);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Reporte de productos vendidos por rango de fechas, ordenado por monto o cantidad."
      />
      ...
      <Card className="max-w-md p-6">
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass} htmlFor="pv-from">Desde</label>
              <input
                id="pv-from"
                type="date"
            ...
            <div>
              <label className={labelClass} htmlFor="pv-to">Hasta</label>
              <input
                id="pv-to"
                type="date"
                className={inputClass}
          ...
          {/* Ordenar por */}
          <div>
            <p className={labelClass}>Ordenar por</p>
            <div className="mt-1 flex overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
              {([
                { value: 'amount', label: 'Monto' },
                { value: 'quantity', label: 'Cantidad' },
              ] as { value: OrderBy; label: string }[]).map((opt) => (
                <button
                  key={opt.value}
                  type="button"
          ...
          <div className="w-full max-w-sm rounded-xl bg-white shadow-xl dark:bg-slate-900">
            ...
            <div className="space-y-4 px-5 py-5">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Productos del <span className="font-medium text-slate-700 dark:text-slate-200">{from}</span> al{' '}
                <span className="font-medium text-slate-700 dark:text-slate-200">{to}</span> · por{' '}
                <span className="font-medium text-slate-700 dark:text-slate-200">{orderBy === 'amount' ? 'monto' : 'cantidad'}</span> en Excel.
              </p>
              <div>
                <label className={labelClass} htmlFor="excel-email-pv">Correo electrónico</label>
                <input id="excel-email-pv" type="email" className={inputClass} placeholder="correo@ejemplo.com"
            ...
            <div className="space-y-4 px-5 py-5">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                ...
                <span className="font-medium text-slate-700 dark:text-slate-200">
                  {orderBy === 'amount' ? 'monto' : 'cantidad'}
                </span>.
              </p>

              <div>
                <label className={labelClass} htmlFor="pv-email">Correo electrónico</label>
                <input
                  id="pv-email"
                  type="email"
                  className={inputClass}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Vista previa del reporte de productos"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {dateRange.from} → {dateRange.to} · por {orderBy === 'amount' ? 'monto' : 'cantidad'}
              </p>
            </div>
          </div>
        ...
        <div className="min-h-0 flex-1 bg-slate-100 dark:bg-slate-950">
          {loading ? (
            ...
            <iframe src={blobUrl} title="Productos vendidos" className="h-full w-full" />
          ) : null}
        </div>
      </div>
    </div>
