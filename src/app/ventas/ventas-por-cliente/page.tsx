...
function triggerDownload(pdf: SalePdf) {
  ...
  a.href = url;
  a.download = pdf.fileName || 'ventas-por-cliente.pdf';
...
export default function VentasClientePage() {
  ...
  useEffect(() => { setPdf(null); setPdfError(null); setExcelError(null); }, [client, from, to]);
  ...
  async function handleExcelDownload() {
    ...
    try {
      const res = await salesApi.salesByClientReportExcel(client!.clientId, from, to);
  ...
  async function handleExcelEmail() {
    ...
    try {
      const res = await salesApi.sendSalesByClientReportExcelEmail(email.trim(), client!.clientId, from, to);
      ...
      <Card className="max-w-md p-6">
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelClass} htmlFor="vc-to">Hasta</label>
              <input
                id="vc-to"
                type="date"
                className={inputClass}
          ...
          <div className="border-t border-slate-200 pt-4 dark:border-slate-700">
            ...
            <div className="flex flex-wrap gap-3">
              <Button variant="secondary" onClick={handleExcelDownload} disabled={!formValid || loadingExcel}>
          ...
          <div className="w-full max-w-sm rounded-xl bg-white shadow-xl dark:bg-slate-900">
            ...
            <div className="space-y-4 px-5 py-5">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Ventas de <span className="font-medium text-slate-700 dark:text-slate-200">{client?.clientDescription}</span>{' '}
                del <span className="font-medium text-slate-700 dark:text-slate-200">{from}</span> al{' '}
                <span className="font-medium text-slate-700 dark:text-slate-200">{to}</span> en Excel.
              </p>
              <div>
                <label className={labelClass} htmlFor="excel-email-vc">Correo electrónico</label>
                <input id="excel-email-vc" type="email" className={inputClass} placeholder="correo@ejemplo.com"
        ...
        >
          <div className="w-full max-w-sm rounded-xl bg-white shadow-xl dark:bg-slate-900">
            ...
            <div className="space-y-4 px-5 py-5">
              ...
              <div>
                <label className={labelClass} htmlFor="vc-email">Correo electrónico</label>
                <input
                  id="vc-email"
                  type="email"
                  className={inputClass}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Vista previa del reporte de ventas por cliente"
      ...
      >
        ...
        <div className="min-h-0 flex-1 bg-slate-100 dark:bg-slate-950">
          {loading ? (
            ...
            <iframe src={blobUrl} title="Ventas por cliente" className="h-full w-full" />
          ) : null}
        </div>
      </div>
    </div>
