...
export default function ReporteVentasPage() {
  ...
  const [excel, setExcel] = useState<SalePdf | null>(null);
  ...
  useEffect(() => { setPdf(null); setPdfError(null); setExcel(null); setExcelError(null); }, [from, to]);
  ...
  async function handleExcelDownload() {
    ...
    setExcelError(null);
    try {
      const res = excel ?? await salesApi.reportExcel(from, to);
      setExcel(res);
  ...
  async function handleExcelEmail() {
    ...
    try {
      const res = await salesApi.sendReportExcelEmail(email.trim(), from, to);
          ...
          <div className="w-full max-w-sm rounded-xl bg-white shadow-xl dark:bg-slate-900">
            ...
            <div className="space-y-4 px-5 py-5">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Reporte del <span className="font-medium text-slate-700 dark:text-slate-200">{from}</span> al <span className="font-medium text-slate-700 dark:text-slate-200">{to}</span> en formato Excel.
              </p>
              <div>
                <label className={labelClass} htmlFor="excel-email-rv">Correo electrónico</label>
                <input id="excel-email-rv" type="email" className={inputClass} placeholder="correo@ejemplo.com"
