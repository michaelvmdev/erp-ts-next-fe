...
import { ApiError, purchasesApi, type SalePdf } from '@/lib/api';
...
function triggerDownload(pdf: SalePdf) {
  ...
  a.href = url;
  a.download = pdf.fileName || 'monto-por-proveedor.pdf';
...
export default function MontoProveedorPage() {
  ...
  async function fetchPdf(): Promise<SalePdf | null> {
    ...
    setPdfError(null);
    try {
      const res = await purchasesApi.suppliersAmountReport(from, to);
      setPdf(res);
      return res;
    } catch (err) {
  ...
  async function handleSendEmail() {
    ...
    setSendResult(null);
    try {
      await purchasesApi.sendSuppliersAmountReportEmail(email.trim(), from, to);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Reporte del monto total de compras por proveedor en un rango de fechas, con IGV incluido."
      />
      ...
      <Card className="max-w-md p-6">
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass} htmlFor="mp-from">Desde</label>
              <input
                id="mp-from"
                type="date"
            ...
            <div>
              <label className={labelClass} htmlFor="mp-to">Hasta</label>
              <input
                id="mp-to"
                type="date"
                className={inputClass}
        ...
        >
          <div className="w-full max-w-sm rounded-xl bg-white shadow-xl dark:bg-slate-900">
            ...
            <div className="space-y-4 px-5 py-5">
              ...
              <div>
                <label className={labelClass} htmlFor="mp-email">Correo electrónico</label>
                <input
                  id="mp-email"
                  type="email"
                  className={inputClass}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Vista previa del reporte de monto por proveedor"
      ...
      >
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-3 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-slate-900 dark:text-white">Monto por proveedor</h2>
        ...
        <div className="min-h-0 flex-1 bg-slate-100 dark:bg-slate-950">
          {loading ? (
            ...
            <iframe src={blobUrl} title="Monto por proveedor" className="h-full w-full" />
          ) : null}
        </div>
      </div>
    </div>
